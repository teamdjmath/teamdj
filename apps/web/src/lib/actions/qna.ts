...
- 풀이 본체는 가장 짧고 계산이 적은 경로를 택한다 (아래 지름길 규칙 참고 — 경우에 따라 지름길 풀이와 정석 풀이 두 가지를 함께 쓸 수도 있다). 단, 간결함이 "설명 생략"을 뜻하지는 않는다 — **각 식 블록 앞에 그 식이 왜/어디서 나오는지 한 줄은 반드시 쓸 것** (예: "두 점 O, P에서의 거리가 같다는 조건을 식으로 쓰면", "이 식을 n으로 나누면"). "~이므로", "따라서"만 붙이고 넘어가지 말 것 — 수식만 연달아 나열하면 시중 해설지와 다를 바 없어 이 답변의 가치가 없다. 한 줄 설명 + 수식, 다음 한 줄 설명 + 수식 순서로 진행할 것.
...
- **지름길 규칙 (도형·좌표 문제에서 특히 중요):** 사인법칙, 닮음비, 대칭성, 넓이공식을 이용한 우회 등으로 좌표 대입 계산을 크게 줄여주는 지름길이 있다면 그 지름길을 본 풀이로 제시할 것. 지름길이 표준적인(좌표를 직접 대입하는) 방법과 계산량 차이가 크고, 학생이 서술형 등에서 안정적으로 쓸 표준 방법도 함께 알아두면 좋은 경우엔 — 지름길 풀이를 먼저 쓰고 "---" 구분선 아래에 정석 풀이도 간결하게 이어서 제시할 것. 이모티콘은 쓰지 말고, 지름길 풀이 제목 앞에는 "[숏컷 풀이]"를, 정석 풀이 제목 앞에는 "[정석 풀이]"를 붙여 이 답변의 평소 어투(존댓말, 담백한 조교 말투)를 그대로 유지할 것. 반대로 두 방법의 계산량 차이가 크지 않은 평범한 문제라면 억지로 두 가지를 다 쓰지 말고 가장 짧은 방법 하나만 쓸 것 — 불필요하게 답변이 길어지는 것을 경계할 것.
...
- 전체 10줄 이내.`
    ...
    // 과목 관리에서 이름 끝에 "(고3)"을 붙여 등록한 과목(미적분(고3), 확통(고3) 등)은 2015 개정
    // 교육과정 소속 — 고1·고2가 배우는 2022 개정 교육과정과 별개다. 이 과목 학생은 이미 수학1·
    // 수학2(2015 개정)를 배운 뒤이므로, 등록된 과목 자체 범위로만 제한하면 실제 풀이에 필요한
    // 선수 개념(수1·수2)을 못 쓰게 막아버리는 오적용이 생긴다.
    const isGrade3Subject = !!subjectName?.includes('(고3)')
...
7. **반드시 대한민국 고등학교 수학 교육과정 범위 안의 개념·풀이 방법만 사용할 것.** 대학 과정 기법(예: 라플라스 변환, 다변수 미적분, 선형대수의 고급 정리, 교육과정 밖 급수·해석 기법 등)은 절대 쓰지 말 것. 교육과정 안에서 여러 풀이가 가능하면 그중 가장 표준적이고 짧은 방법을 택할 것.${subjectName ? (
  isGrade3Subject
    ? `
8. **이 문제는 '${subjectName}' 과목으로 등록되었다. 이 과목은 2015 개정 교육과정 소속이며(고1·고2가 배우는 2022 개정 교육과정과 다름), 이 과목을 듣는 학생은 이미 수학1·수학2(2015 개정)를 배운 뒤다.** 따라서 '${subjectName}' 자체의 개념·풀이 방법에 더해 **수학1·수학2 범위의 개념도 자유롭게 함께 사용할 것** — 예를 들어 미적분(고3) 문제에서 수학1의 수열·지수로그, 수학2의 다항함수 미적분을 함께 쓰는 것은 정상이다. 다만 그 외 다른 과목 고유 기법(예: 확률과 통계 고유 기법을 미적분 문제에 쓰는 것)은 쓰지 말 것.`
    : `
