...
function ContactAction({
  ...
  if (reveal === 'loading') {
    return <span className="text-xs text-zinc-400">확인 중…</span>
  }
  if (reveal === 'error') {
  ...
  if (reveal) {
    ...
    return (
      <div className="space-y-1.5 rounded-lg border border-zinc-200 bg-white px-3 py-2.5">
        <div className="flex items-center gap-2.5">
          <span className="w-11 shrink-0 text-[11px] font-semibold text-zinc-400">학생</span>
        ...
        {parents.map((p, i) => (
          <div key={i} className="flex items-center gap-2.5">
            <span className="w-11 shrink-0 text-[11px] font-semibold text-zinc-400">학부모</span>
            <span className="text-base font-bold text-zinc-900 tabular-nums">
              {p ? formatPhone(p) : '—'}
            </span>
          </div>
  ...
  if (!needsApproval) {
    return (
      <button
        ...
        className="w-full rounded-lg border border-zinc-200 bg-white py-1.5 text-xs font-medium text-zinc-600 hover:border-zinc-400 hover:text-zinc-900 transition-colors"
      >
        연락처 보기
      </button>
    )
  ...
  if (myStatus?.status === 'pending') {
    return (
      <span className="block w-full rounded-lg bg-zinc-100 py-1.5 text-center text-xs text-zinc-500">
        승인 대기 중
      </span>
    )
  }
...
export function RosterClient({ role, classGroups, myStatusByStudent }: Props) {
  ...
  return (
    <div className="space-y-5">
      <div>
        <h1 className="text-xl font-bold text-zinc-950">학생 조회</h1>
      ...
      <input
        ...
        className="w-full rounded-xl border border-zinc-200 bg-white px-3.5 py-2.5 text-sm text-zinc-900 placeholder:text-zinc-400 focus:border-zinc-400 focus:outline-none"
      />
      ...
      {filtered.length === 0 ? (
        ...
        <div className="space-y-3">
          {filtered.map((cls) => {
            ...
            return (
              <div key={cls.id} className="rounded-2xl border border-zinc-200 bg-white overflow-hidden">
                {/* 헤더 — 클릭으로 접기/펼치기 (특별시험 목록과 동일한 패턴) */}
                <button
                  type="button"
                  onClick={() => setExpanded((m) => ({ ...m, [cls.id]: !isOpen }))}
                  className={`flex w-full items-center gap-3 px-5 py-4 text-left hover:bg-zinc-50/60 transition-colors ${isOpen ? 'border-b border-zinc-100' : ''}`}
                >
                  <svg
                  ...
                  <span className="text-xs text-zinc-400">{cls.students.length}명</span>
                </button>
                ...
                {isOpen && (
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 p-4">
                    {cls.students.map((s) => (
                      ...
                      >
                        <div>
                          <p className="text-lg font-bold text-zinc-900 leading-tight">{s.name}</p>
                          <div className="mt-1 flex items-center gap-1.5">
                            {s.school && <span className="text-xs text-zinc-500">{s.school}</span>}
                            {s.grade && (
