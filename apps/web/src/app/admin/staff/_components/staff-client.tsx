    ...
    <div className="space-y-4">
      ...
      <div className="rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900">
        <div className="flex items-center justify-between px-5 pt-5 pb-3">
          ...
          <span className="text-xs text-zinc-400 dark:text-zinc-600">실시간 업데이트</span>
        </div>
        <ul className="divide-y divide-zinc-100 dark:divide-zinc-900">
          {staff.length === 0 && (
            <li className="px-5 py-6 text-center text-sm text-zinc-400 dark:text-zinc-600">등록된 스태프가 없습니다.</li>
          )}
          {staff.map((member) => {
              ...
              <li key={member.userId} className="flex items-center gap-3 px-5 py-4">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-zinc-900 dark:text-zinc-100 flex items-center gap-1.5">
                    {member.name}
                    {isMe && (
                      <span className="text-[10px] font-normal text-zinc-400 dark:text-zinc-600">(나)</span>
                    )}
                  </p>
                ...
                {/* 마지막 업데이트 */}
                {member.updatedAt && (
                  <span className="shrink-0 text-[10px] text-zinc-300 dark:text-zinc-700 hidden sm:block">
