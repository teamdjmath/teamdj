...
import { sendBatchExamPrepKakao, deleteExamPrepSession, deleteReport } from '@/lib/actions/reports'
...
interface Props {
  date: string
  reports: ExamPrepReport[]
}

const sanitize = (s: string) => s.replace(/[/\\:*?"<>|]/g, '_')

export function ExamPrepSessionClient({ date, reports }: Props) {
  ...
  function handleBatchSend() {
    if (!confirm(`${date} 내신대비 리포트를 전체 학부모에게 카카오톡으로 발송하시겠습니까?`)) return
    setBatchResult('')
    setBatchErr('')
    startTransition(async () => {
      const res = await sendBatchExamPrepKakao(date)
  ...
  function handleBatchDelete() {
    if (!confirm(`${date} 내신대비 리포트를 전체 삭제하시겠습니까?\n모든 학생의 데이터와 이미지가 삭제되며 복구할 수 없습니다.`)) return
    setIsDeleting(true)
    startTransition(async () => {
      const result = await deleteExamPrepSession(date)
      if (result.error) {
        alert(result.error)
        setIsDeleting(false)
      } else {
  ...
  function handleDeleteOne(id: string, name: string) {
    if (!confirm(`${name} 학생의 내신대비 리포트를 삭제하시겠습니까?`)) return
    startTransition(async () => {
  ...
  async function handleBatchDownload() {
    ...
    try {
      ...
      saveAs(content, `내신대비리포트_${date}.zip`)
    } catch (e) {
    ...
    <div className="space-y-5">
      ...
      <div className="rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 p-5 space-y-4">
        ...
        <div className="flex items-center justify-between gap-4">
          <div>
            <p className="text-sm font-semibold text-zinc-900 dark:text-zinc-100">카카오톡 전체 발송</p>
            ...
            {batchResult && <p className="mt-1.5 text-xs font-medium text-zinc-700 dark:text-zinc-300">✓ {batchResult}</p>}
          </div>
          <div className="flex items-center gap-2 shrink-0">
            <Link
              href="/admin/reports/exam-prep"
              className="rounded-lg border border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-950 px-4 py-2.5 text-sm font-semibold text-zinc-700 dark:text-zinc-300 hover:bg-zinc-100 dark:hover:bg-zinc-900 hover:border-zinc-300 dark:hover:border-zinc-700 transition-all active:scale-95 shadow-sm"
            >
              리포트 수정
            </Link>
            <button
              ...
              className="rounded-lg border border-red-200 bg-white dark:bg-zinc-900 px-4 py-2.5 text-sm font-medium text-red-600 hover:bg-red-50 transition-all active:scale-95 disabled:opacity-50"
            >
        ...
        <div className="flex items-center justify-between gap-4 border-t border-zinc-100 dark:border-zinc-900 pt-4">
          <div>
            <p className="text-sm font-semibold text-zinc-900 dark:text-zinc-100">이미지 ZIP 다운로드</p>
            {downloadProgress ? (
              <div className="mt-1 flex items-center gap-2">
                <div className="h-1.5 w-36 overflow-hidden rounded-full bg-zinc-100 dark:bg-zinc-900">
                  <div
                    className="h-full bg-zinc-800 dark:bg-zinc-200 transition-all duration-200"
              ...
              <p className="mt-0.5 text-xs text-zinc-400 dark:text-zinc-600">
                파일명: OO고_이름.png · 수정은 리포트 수정 화면에서 같은 학생·날짜로 다시 저장
              </p>
            )}
          </div>
          <button
            ...
            className="shrink-0 rounded-lg border border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-950 px-4 py-2.5 text-sm font-semibold text-zinc-700 dark:text-zinc-300 hover:bg-zinc-100 dark:hover:bg-zinc-900 hover:border-zinc-300 dark:hover:border-zinc-700 transition-all active:scale-95 disabled:opacity-50 shadow-sm"
          >
      ...
      {reports.length === 0 ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {reports.map((r) => (
            <div
              key={r.id}
              className="rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 overflow-hidden hover:border-zinc-300 dark:hover:border-zinc-700 hover:shadow-sm transition-all"
            >
              {/* 썸네일 */}
              ...
              <div className="p-3">
                <div className="flex items-center justify-between gap-2 mb-2">
                  <p className="font-semibold text-sm text-zinc-900 dark:text-zinc-100">
                    {r.studentName || '(학생 없음)'}
                    {r.school && <span className="ml-1.5 text-xs font-normal text-zinc-400 dark:text-zinc-600">{r.school}</span>}
                  </p>
                  {r.kakaoSentAt ? (
                    <span className="shrink-0 rounded-full bg-zinc-900 dark:bg-zinc-100 px-2 py-0.5 text-[10px] font-medium text-white dark:text-zinc-900">발송 완료</span>
                  ) : (
                    <span className="shrink-0 rounded-full bg-zinc-100 dark:bg-zinc-900 px-2 py-0.5 text-[10px] font-medium text-zinc-500 dark:text-zinc-500">미발송</span>
                  )}
                </div>
                <div className="flex items-center gap-3 text-xs">
                  ...
                  <button
                    type="button"
                    onClick={() => handleDeleteOne(r.id, r.studentName)}
                    disabled={pending}
