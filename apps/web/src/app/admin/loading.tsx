export default function AdminLoading() {
  return (
    <div className="flex min-h-screen bg-zinc-50 dark:bg-zinc-950">
      {/* 데스크탑 사이드바 스켈레톤 */}
      <aside className="hidden md:flex md:w-56 md:flex-col md:fixed md:inset-y-0 border-r border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900">
        <div className="flex h-14 items-center px-5 border-b border-zinc-100 dark:border-zinc-900">
          <div className="h-4 w-20 rounded bg-zinc-100 dark:bg-zinc-900 animate-pulse" />
        </div>
      ...
      <div className="md:hidden fixed top-0 left-0 right-0 z-30 flex items-center justify-between border-b border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 px-4 h-14">
        <div className="h-4 w-16 rounded bg-zinc-100 dark:bg-zinc-900 animate-pulse" />
        <div className="h-8 w-8 rounded-lg bg-zinc-100 dark:bg-zinc-900 animate-pulse" />
      </div>
      ...
      <div className="flex-1 md:pl-56">
        <main className="p-5 pt-20 md:pt-5 max-w-5xl space-y-4">
          ...
          {Array.from({ length: 3 }).map((_, i) => (
            <div key={i} className="rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 p-6 space-y-3 animate-pulse">
