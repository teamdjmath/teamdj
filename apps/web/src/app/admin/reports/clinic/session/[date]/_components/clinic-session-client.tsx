...
import { sendBatchClinicKakao, deleteClinicSession, deleteReport } from '@/lib/actions/reports'
...
export function ClinicSessionClient({ date, reports }: Props) {
  ...
  function handleBatchSend() {
    if (!confirm(`${date} 클리닉 리포트를 전체 학부모에게 카카오톡으로 발송하시겠습니까?`)) return
    setBatchResult('')
    setBatchErr('')
    startTransition(async () => {
      const res = await sendBatchClinicKakao(date)
  ...
  function handleBatchDelete() {
    if (!confirm(`${date} 클리닉 리포트를 전체 삭제하시겠습니까?\n모든 학생의 데이터와 이미지가 삭제되며 복구할 수 없습니다.`)) return
    setIsDeleting(true)
    startTransition(async () => {
      const result = await deleteClinicSession(date)
      if (result.error) {
        alert(result.error)
        setIsDeleting(false)
      } else {
  ...
  function handleDeleteOne(id: string, name: string) {
    if (!confirm(`${name} 학생의 클리닉 리포트를 삭제하시겠습니까?`)) return
    startTransition(async () => {
  ...
  async function handleBatchDownload() {
    ...
    try {
      ...
      saveAs(content, `클리닉리포트_${date}.zip`)
    } catch (e) {
          ...
          <div>
            ...
            {downloadProgress ? (
              ...
              <p className="mt-0.5 text-xs text-zinc-400 dark:text-zinc-600">
                파일명: OO고_이름.png · 수정은 같은 날짜로 엑셀 재업로드 시 덮어쓰기
              </p>
            )}
          </div>
          <button
