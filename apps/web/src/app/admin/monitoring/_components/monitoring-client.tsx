...
function BarChart({ data, color = 'bg-zinc-900 dark:bg-zinc-100' }: { data: Array<{ label: string; value: number }>; color?: string }) {
  const max = Math.max(...data.map((d) => d.value), 1)
  return (
    <div className="flex items-end gap-1 h-24">
      {data.map((d, i) => (
        <div key={i} className="flex-1 flex flex-col items-center justify-end gap-1 min-w-0 h-full">
          <span className="text-[9px] text-zinc-400 dark:text-zinc-600 tabular-nums leading-none">
            {d.value > 0 ? d.value : ''}
          </span>
          <div
            className={`w-full rounded-t ${d.value > 0 ? color : 'bg-zinc-100 dark:bg-zinc-900'}`}
            style={{ height: `${Math.max((d.value / max) * 75, d.value > 0 ? 6 : 2)}%` }}
          />
          <span className="text-[9px] text-zinc-400 dark:text-zinc-600 truncate leading-none">{d.label}</span>
        </div>
      ))}
    </div>
  )
  ...
  return (
    <div className="rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 p-5">
      <div className="mb-4">
        ...
        {sub && <p className="mt-0.5 text-xs text-zinc-400 dark:text-zinc-600">{sub}</p>}
      </div>
      {children}
    </div>
  )
...
export function MonitoringClient({ stats, aiUsage, aiFeedback, checkedAt }: Props) {
  ...
  if (!stats) {
    return (
      <div className="rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 py-20 text-center">
        <p className="text-sm text-zinc-400 dark:text-zinc-600 mb-2">지표를 불러오지 못했습니다.</p>
        <p className="text-xs text-zinc-400 dark:text-zinc-600">마이그레이션 050이 적용되었는지 확인해주세요. (monitoring_behavior_stats)</p>
      </div>
    )
  }
  ...
  return (
    <div>
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-zinc-950 dark:text-zinc-50">모니터링</h1>
        ...
        <button
          ...
          onClick={refresh}
          disabled={refreshing}
          className="rounded-lg border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 px-3.5 py-2 text-sm text-zinc-600 dark:text-zinc-400 hover:bg-zinc-50 dark:hover:bg-zinc-950 disabled:opacity-50 transition-colors"
        >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-4">
        <div className="rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 p-5">
          <p className="text-xs text-zinc-400 dark:text-zinc-600 mb-1">리포트 알림율 (30일)</p>
          <p className="text-2xl font-bold text-zinc-950 dark:text-zinc-50 tabular-nums">
        ...
        <div className="rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 p-5">
          <p className="text-xs text-zinc-400 dark:text-zinc-600 mb-1">공지 열람율 (30일)</p>
          <p className="text-2xl font-bold text-zinc-950 dark:text-zinc-50 tabular-nums">
            {stats.notices_30d.read_rate !== null ? `${stats.notices_30d.read_rate}%` : '—'}
          </p>
          <p className="mt-1 text-xs text-zinc-400 dark:text-zinc-600">발행 {stats.notices_30d.total}건 기준</p>
        </div>
        <div className="rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 p-5">
          <p className="text-xs text-zinc-400 dark:text-zinc-600 mb-1">로그인 (7일)</p>
          <p className="text-2xl font-bold text-zinc-950 dark:text-zinc-50 tabular-nums">
          ...
          <p className="mt-1 text-xs text-zinc-400 dark:text-zinc-600">성공 {stats.login_7d.success} · 실패 {stats.login_7d.failed}</p>
        </div>
      </div>
      ...
      <div className="mb-4 rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 p-5">
        ...
        {!aiUsage || aiUsage.calls === 0 ? (
          <p className="py-4 text-center text-xs text-zinc-400 dark:text-zinc-600">이번 달 AI 호출 기록이 없습니다.</p>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div>
              <p className="text-xs text-zinc-400 dark:text-zinc-600">호출 수</p>
              <p className="mt-1 text-xl font-bold text-zinc-950 dark:text-zinc-50 tabular-nums">{aiUsage.calls}건</p>
              <p className="mt-0.5 text-xs text-zinc-400 dark:text-zinc-600">힌트 {aiUsage.hintCalls} · 최종답 {aiUsage.fullCalls}</p>
            </div>
            <div>
              <p className="text-xs text-zinc-400 dark:text-zinc-600">예상 요금</p>
              <p className="mt-1 text-xl font-bold text-zinc-950 dark:text-zinc-50 tabular-nums">{formatKrw(aiUsage.costKrw)}</p>
              <p className="mt-0.5 text-xs text-zinc-400 dark:text-zinc-600">토큰 {aiUsage.totalTokens.toLocaleString('ko-KR')}개</p>
            </div>
            <div>
              <p className="text-xs text-zinc-400 dark:text-zinc-600">평균 비용 / 건</p>
              <p className="mt-1 text-xl font-bold text-zinc-950 dark:text-zinc-50 tabular-nums">
                {aiUsage.avgFullKrw !== null ? formatKrw(aiUsage.avgFullKrw) : '—'}
                <span className="ml-1 text-xs font-normal text-zinc-400 dark:text-zinc-600">최종답</span>
              </p>
              <p className="mt-0.5 text-xs text-zinc-400 dark:text-zinc-600">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <MetricCard title="출석체크 사용 빈도" sub="최근 14일 · 일별 기록 수">
          {stats.attendance_daily.length === 0 ? (
            <p className="py-8 text-center text-xs text-zinc-400 dark:text-zinc-600">기록이 없습니다.</p>
          ) : (
